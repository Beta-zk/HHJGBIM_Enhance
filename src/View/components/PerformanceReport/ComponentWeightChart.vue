...
const initCharts = () => {
  ...
  monthChartInstance?.setOption({
    title: { text: '月度深化重量', left: 'center', textStyle: { color: '#e2e8f0', fontSize: 16, fontWeight: 'bold' } },
  ...
  quarterChartInstance?.setOption({
    title: { text: '季度深化重量', left: 'center', textStyle: { color: '#e2e8f0', fontSize: 16, fontWeight: 'bold' } },
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
    grid: { left: '3%', right: '3%', bottom: '5%', top: '20%', containLabel: true },
...
<style scoped>
/* 延续通用图表容器结构 */
.chart-container { display: flex; flex-direction: column; justify-content: space-evenly; height: 100%; gap: 40px; }
