...
import { domMaster, type IDraggablePanel } from './DomMaster';
 ...
 * @description 项目状态生命周期增强中间件。基于项目实体字典为表格行匹配状态，注入语义化色系指示器、
 * 悬浮式状态筛选面板，并在筛选时重算 vxe 表格页脚汇总。所有宿主页面 DOM 操作委托给 DomMaster，
 * 本类仅保留业务编排与数据重组逻辑。
 */
export class ProjectStateEnhance {
    ...
    /**
     * @method init
     * @description 注入增强样式，预热项目实体字典（含爬虫探活降级），并注册仓储汇总接口的响应拦截器。
     */
    public init(): void {
        domMaster.injectStyle(this.STYLE_TAG_ID, STATE_ENHANCE_STYLES);
    ...
    private checkBlacklistMatch(): boolean {
        return domMaster.isUrlMatch(this.BLACKLIST_ROUTES);
    }
    ...
    private checkWhitelistMatch(): boolean {
        return domMaster.isUrlMatch(this.WHITELIST_ROUTES);
    }
    ...
    private get isFilterActive(): boolean {
        return domMaster.getElementById(this.FILTER_CONTAINER_ID) !== null;
    }
    ...
    private cleanupAll(): void {
        ...
        domMaster.querySelectorAll<HTMLElement>('.hhjg-state-row-enhanced').forEach(el => {
            domMaster.removeClass(el, 'hhjg-state-row-enhanced');
            el.style.display = ''; 
        });

        domMaster.querySelectorAll<HTMLElement>('.hhjg-state-cell-enhanced').forEach(el => {
            domMaster.removeClass(el, 'hhjg-state-cell-enhanced');
        });

        domMaster.querySelectorAll<HTMLElement>('.hhjg-state-indicator').forEach(indicator => indicator.remove());
    }
    ...
    private injectFilterUI(originalJson?: any): void {
        ...
        domMaster.waitForElement('.cs-z-page-main-content').then(wrapperEl => {
            ...
            this.filterPanel = domMaster.createDraggablePanel({
                ...
                onReady: (container) => {
                    sortedStates.forEach(state => {
                        ...
                        const btn = domMaster.createElement('div', {
    ...
    private activateGlobalObserver(): void {
        domMaster.observeDom((mutations) => {
    ...
    private scanAndApply(): void {
        ...
        const allTrs = domMaster.querySelectorAll<HTMLElement>('table tbody tr');
        ...
        const parsedRows: RowMeta[] = allTrs.map(tr => {
            ...
            const entries = domMaster.getRowCellEntries(tr);
        ...
        parsedRows.forEach(meta => {
            ...
            if (finalState) {
                ...
                domMaster.addClass(tr, 'hhjg-state-row-enhanced');

                if (targetElement) {
                    domMaster.addClass(targetElement, 'hhjg-state-cell-enhanced');
                    domMaster.upsertIndicator(targetElement, this.getBadgeColor(finalState));
                }
            } else {
                domMaster.removeClass(tr, 'hhjg-state-row-enhanced');
                tr.style.display = ''; 
                tr.querySelectorAll('.hhjg-state-cell-enhanced').forEach(el => {
                    domMaster.removeClass(el as HTMLElement, 'hhjg-state-cell-enhanced');
    ...
    private updateFooter(forceReset: boolean = false): void {
        ...
        domMaster.recomputeVxeFooterTotals(isAllActive);
    }
}
